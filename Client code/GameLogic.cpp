...
// Simple automatic ship placement (for testing)
void GameLogic::PlaceShips() {
    ...
    while (idx < 10) {
        ...
        if (placed) idx++;
            ...
            // If placement fails, restart completely
            memset(myField, 0, sizeof(myField));
            idx = 0;
        }
    }
...
void GameLogic::DrawFields() const {
    ...
    std::cout << "=== YOUR FIELD (Player " << playerNumber << ") ===\n";
    ...
    for (int y = 0; y < 10; ++y) {
        ...
        for (int x = 0; x < 10; ++x) {
            ...
            switch (myField[y][x]) {
            case 0: ch = '.'; break;
            case 1: ch = '#'; break;  // ship
            case 2: ch = 'X'; break;  // hit
            case 3: ch = 'o'; break;  // miss
            default: ch = '?';
            }
            std::cout << ch << ' ';
        }
...
// Check if the ship containing (x,y) is completely destroyed (all cells are 2)
bool GameLogic::IsShipSunk(int x, int y) const {
    // BFS over connected cells that are part of the ship (value 1 or 2)
    ...
    while (head < tail) {
        ...
        if (myField[cy][cx] == 1) allHit = false; // still undamaged part
        int dx[] = { 1, -1, 0, 0 };
        int dy[] = { 0, 0, 1, -1 };
        for (int i = 0; i < 4; ++i) {
...
// Mark all neighbouring cells of a sunken ship as misses
void GameLogic::MarkAroundShip(int x, int y) {
    // We'll BFS over the whole ship and mark all 8 neighbours as miss (if empty)
    bool visited[10][10] = { false };
    int queueX[100], queueY[100];
    int head = 0, tail = 0;
    queueX[tail] = x; queueY[tail] = y; tail++;
    ...
    while (head < tail) {
        ...
        // Find other parts of the ship (already hit)
